...
class WebSecurityTests(unittest.TestCase):
    ...
    def test_dashboard_summary_refreshes_fast_and_heavy_chart_refreshes_slower(self):
        ...
        self.assertIn("const netSpecterLiveIntervalSeconds = netSpecterFastMode ? 30 : 10;", source)
        self.assertIn("data-live-countdown", source)
        self.assertIn("function hasLiveSpeedWidgets()", source)
        self.assertIn("next live update in", source)
        self.assertIn("Fast mode is on. Graph loads once and only refreshes when requested.", source)
        self.assertIn('id="dashboardGraphRefresh"', source)
        self.assertIn("loadDashboardTraffic();", source)
        self.assertIn('setDashboardGraphStatus("chart library not loaded");', source)
