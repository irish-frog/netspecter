...
class WebSecurityTests(unittest.TestCase):
    ...
    def test_uptime_kuma_is_installed_and_exposed_in_dashboard(self):
        source = (SOURCE_DIR / "app.py").read_text()
        installer = (SOURCE_DIR / "install.sh").read_text()
        example = json.loads((SOURCE_DIR / "config.example.json").read_text())
        readme = (SOURCE_DIR / "README.md").read_text()
        self.assertEqual("http://127.0.0.1:3001", self.module.DEFAULT_CONFIG["kuma_url"])
        self.assertEqual("http://127.0.0.1:3001", example["kuma_url"])
        self.assertIn('INSTALL_KUMA="${INSTALL_KUMA:-1}"', installer)
        self.assertIn("louislam/uptime-kuma:1", installer)
        self.assertIn("--name uptime-kuma", installer)
        self.assertIn("-p 3001:3001", installer)
        self.assertIn("def kuma_status", source)
        self.assertIn("Uptime Kuma", source)
        self.assertIn("kuma_url", source)
        self.assertIn("http://SERVER-IP:3001", readme)
