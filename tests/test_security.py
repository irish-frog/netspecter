...
class WebSecurityTests(unittest.TestCase):
    ...
    def test_dashboard_summary_refreshes_fast_and_heavy_chart_refreshes_slower(self):
        ...
        self.assertIn("setInterval(refreshLiveSpeeds, 10000);", source)
        self.assertIn("const dashboardFastMode =", source)
        self.assertIn("const netSpecterFastMode =", source)
        self.assertIn("Fast mode is on. Traffic graph loads only when requested.", source)
        ...
        self.assertIn("if (!netSpecterFastMode && document.querySelector('[data-live-ip][data-live-field], [data-live-network][data-live-field]'))", source)
